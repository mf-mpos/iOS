...
typedef enum {
    ...
    MF_RET_FAIL_MPOSTRADE = -120,       //!> 组包失败: MPOS交易处理
    ...
    MF_RET_MKEY_KVC,                    //!> 主密钥下载(返回KVC)
    MF_RET_WKEY_LOAD,                   //!> 工作密钥下载
    MF_RET_INPUT_PIN = 110,                 //!> 密码输入
    MF_RET_CALC_MAC,                  //!> MAC计算
    MF_RET_PIN_ENCRYPT,                //!> PIN加密
    MF_RET_TRACK_OPER,                  //!> 磁道操作(加密、解密)
    MF_RET_OPEN_CARD = 120,            //!> 开启读卡器
    ...
    MF_RET_MPOS_INFO,                   //!> MPOS交易提示
    MF_RET_UNIPAY21FIELD,               //!> 获取21号文字段
    MF_RET_SHOW_BARCODE,                //!> 显示二维码
    MF_RET_INPUT_AMOUNT,                //!> 输入金额
    MF_RET_WORKKEY_HKRT,                //!> 下载工作密钥（海科）
    MF_RET_CARD_POWER,                  //!> 卡上下电
    MF_RET_CARD_EXCHANGE,               //!> 卡通讯
    MF_RET_DIPLAY_TEXT  ,               //!> MPOS文本显示
    MF_RET_RFGROUND_STRENGTH,           //!> 非接场强
    MF_RET_SHOW_QRCODE,                 //!> 显示QR码
    MF_RET_KEK_DOWNLOAD_ZF,             //!> 中付KEK下载
} MFEU_POS_RESULT;
...
/// 磁道加密/解密
typedef enum {
    MF_TRACK_ENCRYPT = 0x00,        //!< 加密
    MF_TRACK_DECRYPT = 0x01,        //!< 解密
} MFEU_TRACK_OPER;

typedef enum{
    MF_MPOS_RESP_CANCEL = 0,            //!> 用户取消
    MF_MPOS_RESP_MAGTEK,                //!> 磁条卡交易
    MF_MPOS_RESP_ICC,                   //!> IC卡交易
    MF_MPOS_RESP_RFID,                  //!> 非接交易
    MF_MPOS_RESP_FORCE_IC,              //!> 强制IC卡
    MF_MPOS_RESP_TIMEOVER,              //!> 刷卡超时
    MF_MPOS_RESP_CARD_FAIL,             //!> 读卡失败
} MFEU_MPOS_RESP;
...
typedef enum {
    ...
    MF_COMBINED = (0x01 | 0x02),        //!< 读磁条+IC
    MF_READ_RFID = 0x04,                //!< 读取RFID
    MF_READ_ALL = (0x01 | 0x02 | 0x04)
} MFEU_READCARD_TYPE;
...
typedef enum {
    ...
    MF_PINREQ_AUTO = 0x02,              //!> 免密智能判断(当小于免密金额时，非接不输密，IC卡需要输密)
} MFEU_PINREQ;

/// 是否允许降级
typedef enum {
    MF_FAILBACK_NO = 0x00,              //!> 0 不允许降级
    MF_FAILBACK_YES = 0x01,             //!> 1 允许降级
} MFEU_FAILBACK;

// MPOS交易提示
typedef enum {
    MF_INFO_NONE = 0xFF,
    MF_INFO_CUSTOM = 0x00,                  //!< 0-按照自定义内容提示
    MF_INFO_WAITCARD = 0x01,                //!< 1-等待刷卡
    MF_INFO_READCARD = 0x02,                //!< 2-正在读卡
    MF_INFO_WAITPIN = 0x03,                 // 3-等待用户输入密码
} MFEU_MSR_INFO;

typedef enum {
    MF_PARAM_NONE = 0x00,
    MF_PARAM_TRMNNO,                    //!> 终端号
    MF_PARAM_MERCHNO,                   //!> 商户号
    MF_PARAM_BATCHNO,                   //!> 批次号
    MF_PARAM_FLOWNO,                    //!> 流水号
} MFEU_DATAPARAM;

/// MPOS文本显示返回
typedef enum {
    MF_POSRESP_OK = 0x00,                  //!< 确认键
    MF_POSRESP_CANCEL = 0x01,              //!< 取消键
    MF_POSRESP_TIMEOUT = 0x02              //!< 超时
} MFEU_MSR_POSRESP;

/// MPOS文本显示返回
typedef enum {
    MF_DISPRESP_OK = 0x01,                  //!< 确认键
    MF_DISPRESP_CANCEL = 0x02,              //!< 取消键
    MF_DISPRESP_TIMEOUT = 0x03              //!< 超时
} MFEU_MSR_DISPRESP;
