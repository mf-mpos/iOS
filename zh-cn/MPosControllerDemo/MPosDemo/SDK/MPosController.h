...
// TUSN
static NSString* UNIPAY_KEY_TUSN = @"TUSN";
// 随机因子
static NSString* UNIPAY_KEY_RANDFACTOR = @"randFactor";
static NSString* UNIPAY_KEY_RANDFACTOR_STRING = @"stringFactor";
// SNKEY
static NSString* UNIPAY_KEY_SNKEY = @"SNKey";
...
/// 仅搜索魔方设备
-(void) scanMFDevice:(NSInteger)timeout;
...
/// 工作密钥下载请求
/**
 * @param key       [IN]    pin 或 mac 或 磁道密钥(长度为40的字符串，32位密钥＋8位kvc)
 * @param keyType   [IN]    密钥类型
 * @param index     [IN]    密钥索引
 * @return int      小于0，表示蓝牙未准备好
 * @see 消息回调函数: MPosDelegate::didLoadWorkKeyExResp:
 * @see loadKek:keyLength:
 * @see loadMainKey:encryptMethod:keyIndex:keyLength:
 * @see loadWorkKey:keyIndex:keyLength:
 * @see setKeyIndex:
 ...
 * @return int      小于0，表示蓝牙未准备好
 */
-(NSInteger) trackOper: (MFEU_TRACK_OPER) type withData: (NSString *)data;
-(NSInteger) trackOperData: (MFEU_TRACK_OPER) type withData: (NSString *)data;
...
/// 设置商终号请求
/**
 * @param pszShop    [IN]    商户号(长度15位)
 * @param pszDevice  [IN]    终端号(长度8位)
 * @return int      小于0，表示蓝牙未准备好
 */
 ...
 * @param isEncrypt        返回数据是否加密, isEncryt==YES送二磁道密文
 * @param showMsg          MPOS上显示信息
 * @param transLogNo        交易流水号
 * @see openCardReader:aMount:timeOut:readType:showMsg:
 ...
 */
-(void) cardRead:(NSInteger)amountFen transType: (MFEU_TRADE_TYPE)eTransType timeOutSecond:(NSInteger) timeout isEncrypt: (BOOL)isEncrypt showMsg: (NSString *)showMsg transLogNo: (NSString *)seq;;
 ...
 * @param pszTradeType  交易类型 LL20
 * @param pszMoney      交易金额 LL20
 * @param tradeType     交易类型
 * @param pboc          PBOC 流程指示
 * @param ecash         是否支持电子现金
 * @param online        强制联机标识
 * @param pinreq        PIN输入指示
 * @param nPwdMaxLength 密码的最大长度 <=0x0C
 * @param nPwdTimeout   密码超时时间(1-255，单位：秒）
 * @param failback      是否允许降级
 * @param flowNo        流水号
 * @param orderNo       订单号
 * @param tagList       自定义tag列表（默认NULL）
 */
 ...
 * @param pszMoney      交易金额 LL20
 * @param cFactory      厂商ID
 * @param nAuthAmt      IC卡授权金额
 * @param nOtherAmt     IC卡其他金额
 * @param tradeType     交易类型
 * @param pboc          PBOC 流程指示
 * @param ecash         是否支持电子现金
 * @param online        强制联机标识
...
-(int) getUnipay21field: (NSString *)cardNo;

-(int) getField59data: (NSString *)cardNo;
...
-(int) showBarcode: (NSString *)content
           timeout: (unsigned char) timeout;

-(int) inputAmount: (NSString *)msg
           timeout: (unsigned char) timeout;

-(int) loadWorkKeyHkrt: (NSString *)data;

-(int) setDisplayText: (unsigned char)t text: (NSString *)text;

-(int) setRFGroundStrength: (unsigned char)v;

-(int) showQrcode: (NSString *)content
          timeout: (unsigned char) timeout;

-(int) DukptLoadKey: (MFEU_DUKPT_ALG) alg withIndex: (MFEU_KEY_INDEX)index withKey: (NSString *)key withKsn: (NSString*)ksn;

-(int) DukptGetKey: (MFEU_KEY_INDEX)index withType: (MFEU_DUKPT_TYPE)type;

-(int) DukptGenKey: (MFEU_KEY_INDEX)index;

-(int) DukptDes: (MFEU_DUKPT_TYPE)type withOper: (MFEU_DUKPT_OPER)oper withMethod: (MFEU_DUKPT_METHOD)method withData: (NSString*)data;
...
- (void)didFoundCBPeripheral: (CBPeripheral *)peripheral;
...
/// 仅返回魔方蓝牙设备列表
-(void) didStopScanDevice: (NSDictionary *)deviceList;
...
-(void) didLoadWorkKeyHkrtResp: (MFEU_MSR_RESP)resp;
 ...
 * @param track2Length  二磁长度
 * @param track3Length  三磁长度
 * @param track2Data    二磁数据
 * @param track3Data    三磁数据
 * @param randomNumber  随机数
 * @param data55        交易相关数据
 * @param failback      是否降级
 * @param pwdLength     密码长度
 * @param pinBlock      PINBLOCK
 * @param serialNum     卡片序列号
**/
-(void)didMPosTradeResp:(MFEU_MPOS_RESP)cardResp
        ...
        andTrack3Length:(int)track3Length
          andTrack2Data:(NSString *)track2Data
          andTrack3Data:(NSString *)track3Data
        andRandomNumber:(NSString *)randomNumber
             andIccData:(NSString *)data55
            andFailback:(MFEU_FAILBACK)failback
      andPasswordLength:(int)pwdLength
...
-(void)didShowBarcodeResp:(MFEU_MSR_RESP)resp;

-(void)didInputAmountResp:(MFEU_MSR_POSRESP)resp content:(NSString *)info;

-(void)didSetRFGroundStrengthResp: (MFEU_MSR_RESP)resp;
